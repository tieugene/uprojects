...
bool	MXL_Parse (Pdata &buffer, MXL &data)	{
	bool	bfirstole = true;
	BYTE	*ptr = buffer.data, *eptr = buffer.data + buffer.size;	//, *bmax = buffer.data + buffer.size;
	WORD	i;

	btmp = buffer.data;
	TotalS = TotalC = data.TotalStrings = data.TotalChars = 0;	// reset string counters
	...
	if (*data.Font.Qty) {
		...
		for (i = 0; i < *data.Font.Qty; i++)	{		// count fontname strings
			if (*data.Font.Prop[i].FontName) {
				TotalC += strlen(data.Font.Prop[i].FontName);
				TotalS++;
			}
		}
	}
...
//if (ver == ver2)	cerr << (ptr - buffer.data) << ": VSect.Qty:" << *data.VSect.Qty << endl;
	...
	//
	data.TotalChars = TotalC;
	data.TotalStrings = TotalS;
	//
	return (true);
}
