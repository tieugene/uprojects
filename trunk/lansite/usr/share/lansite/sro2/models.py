...
class	Stage(models.Model):
	...
	no		= models.PositiveSmallIntegerField(null=False, blank=False, verbose_name=u'№')
	...
	def	asstr(self):
		return u'%s: %02d %s' % (self.srotype.name, self.no, self.name)
...
class	Job(models.Model):
	...
	def	asstr(self):
		return u'%d: %s' % (self.okdp, self.name)
...
class	SpecialityStage(models.Model):
	...
	def	asstr(self):
		return u'%s: %s' % (self.speciality.name, self.stage.name)
...
class	OrgSro(models.Model):
	...
	def	asstr(self):
		return u'%s: %s' % (self.sro.name, self.org.name, )
...
class	StageList(models.Model):
	...
	def	asstr(self):
		return u'%s: %s' % (self.orgsro, self.type.name)
...
class	PermitStageJob(models.Model):
	...
	def	asstr(self):
		return u'%s: %s' % (self.permitstage, self.job)
...
class	Statement(models.Model):
	...
	def	asstr(self):
		return u'%s: от %s' % (self.stagelist, self.date)
