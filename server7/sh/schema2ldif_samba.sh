...
# convert OpenLDAP samba schema file to LDIF file
#
...
if [ -x $slaptest ] ; then
	schemaFile="/etc/openldap/schema/samba.schema"
	#localdir=$(pwd)
	if [ -r $schemaFile ] ; then
		targetFile="/etc/openldap/schema/samba.ldif"
...
include /etc/openldap/schema/samba.schema" > tmp.conf
			# convert
			$slaptest -f tmp.conf -F $tmpDir
			# 3. rename and sanitize
			cd cn\=config/cn\=schema
			filenametmp="cn={3}samba.ldif"
			sed -r -e  's/^dn: cn=\{3\}(.*)$/dn: cn=\1,cn=schema,cn=config/' \
				-e 's/cn: \{3\}(.*)$/cn: \1/' \
