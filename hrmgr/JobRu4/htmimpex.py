...
##					self.buffer[19] = 1	# New
		...
		if (self.status == 0):
			...
			if (tag == "table"):
				...
				if (self.level == 0):		# end of main table
					...
					model.AddRecord(self.Model, self.buffer)
			elif (tag == "tr"):
				pass
			elif (tag == "td"):
				self.InCell = False
	...
	def	handle_data(self, d):
		if ((self.status) and (self.level > 1) and (self.InCell)):
			s = d.strip()
			if len(s):
				self.__DataProcess(s)

	def	__DataProcess(self, text):
		if (self.level == 2):
			if (self.row == 2):			# Date, Title
				if (not self.Bold):
					self.buffer[1] = QtCore.QDateTime().fromString(text[:19], "dd.MM.yyyy hh:mm:ss")
				else:
	...
	def	__X(self, s, n):
		'''
		XCode input string from cp1251 into unicode
...
def	Import(widget, mod, fileName):
	...
	MyHTMLParser().LetsGo(open(fileName).read(), mod)
	model.CommitMassAdd(mod)
